...
## Mental model & goals
- The CLI (and eventual TUI) observes `$reel`, which jumps every time the Supabase table is touched, the cursor crawls, and the perspective cache resolves. Those jumps are fine so long as we know when the underlying work has completed.
...
- Drafted this spec (`ax/reel/spec.md`) and TODO list (`ax/reel/TODO.md`) to reflect the new focus on tracking outstanding requests rather than filtering emissions.
...
- Without instrumentation in `src/libs/reel/cli.js` that watches the register, we cannot prove that the counter reaches zero before the next command or before exiting.

## Next steps reference
1. Build the outstanding request tracker described in the TODO, hook it into `src/libs/reel/shell.js`/`core.js`, and expose it to `src/libs/reel/cli.js` so the CLI can log the register with every snapshot.
...
3. Use the CLI instrumentation as the proof-of-life: it should log only once the register is zero and the rest of the story is visible in the `$reel` snapshot.
